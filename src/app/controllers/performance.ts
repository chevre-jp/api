 ...
 * パフォーマンスコントローラー
 */
import { Models, PerformanceStatusesModel } from '@chevre/domain';
...
export async function search(req: Request, res: Response) {
    ...
    // 空席情報を追加
    let performanceStatuses: PerformanceStatusesModel.PerformanceStatuses;
    try {
        performanceStatuses = await PerformanceStatusesModel.find();
    } catch (error) {
        console.error(error);
    }
...
async function addFilmConditions(andConditions: any[], section: string | null, words: string | null): Promise<void> {
    ...
    if (words !== null) {
        ...
        const orConditions = trimmedWords.split(' ').filter((value) => (value.length > 0)).reduce(
            (a: any[], word) => {
                return a.concat(
                    { 'name.ja': { $regex: `${word}` } },
                    { 'name.en': { $regex: `${word}` } }
