    ...
    try {
        ...
        if (typeof req.project?.id === 'string' && req.project.id !== ANY_PROJECT_ID) {
            ...
            if (memberPermissionReadable) {
                ...
                searchPermissionsResult = await chevre.service.iam.searchPermissions({
...
function checkProjectMember(params: {
    ...
    return async (repos: {
        member: chevre.repository.Member;
    }) => {
        let isMember = false;
        ...
        if (members.length > 0) {
            ...
            isMember = member.member.typeOf === chevre.factory.personType.Person
